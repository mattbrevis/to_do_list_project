      ...
      body: ListView.builder(
        ...
        switch (listTasks[index].status) {
          case 0:
            ...
            iconIndexedTask = const Icon(Icons.pending_actions_rounded, color: Colors.red,size: 50);
            break;
            case 1:
            statusIndexTask = statusTask[1];
            iconIndexedTask = const Icon(Icons.task_rounded, color: Colors.green,size: 50);
            break;
            case 2:
            statusIndexTask = statusTask[2];
            iconIndexedTask = const Icon(Icons.run_circle_outlined, color: Colors.orange,size: 50);
            break;          
        }
        return SizedBox(
          ...
          child: ListTile(
            onTap: (){
              Navigator.push(
    context,
    MaterialPageRoute(builder: (context) => TaskPage(task: listTasks[index],)),
  );
            },
            leading: iconIndexedTask,
            title: Text(listTasks[index].titleTask.toUpperCase(),style: Theme.of(context).textTheme.titleLarge),
            subtitle:
                Text('${listTasks[index].descriptionTask.toString()} \n$statusIndexTask', style: Theme.of(context).textTheme.titleMedium),
